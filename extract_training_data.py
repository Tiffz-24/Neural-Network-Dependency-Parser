    ...
    def get_input_representation(self, words, pos, state):
        ...
        for i in range(3):
            if len(state.buffer) <= i:
                output[index] = self.word_vocab['<NULL>']
            else:
                buffer_val = words[state.buffer[-1 - i]]
                pos_val = pos[state.buffer[-1-i]]

                if pos_val == 'NNP':
                    output[index] = 1
                elif pos_val == 'CD':
                    output[index] = 0
                elif buffer_val == "NULL":
                    output[index] = self.word_vocab['<NULL>']
                elif buffer_val == None:
                    output[index] = 3
                elif buffer_val:
                    if buffer_val not in self.word_vocab.keys():
                        output[index] = self.word_vocab['<UNK>']
                    else:
                        output[index] = self.word_vocab[buffer_val]
        ...
        for i in range(3):
            if len(state.stack) <= i:
                ...
                stack_val = words[state.stack[-1 - i]]
                pos_val = pos[state.stack[-1 - i]]
                ...
                if pos_val == 'NNP':
                    ...
                    if stack_val not in self.word_vocab.keys():
                            output[index] = self.word_vocab['<UNK>']
                    else:
                        output[index] = self.word_vocab[stack_val]
